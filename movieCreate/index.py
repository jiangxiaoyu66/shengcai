...
LastEditTime: 2023-08-01 17:05:18
FilePath: /shengcaicopy/movieCreate/index.py
Description: 
'''
...
audio_path = os.path.join(current_folder, "input/yinpin.wav")
subtitle_path = os.path.join(current_folder, "input/zimu.srt")
...
def create_video_segment(sentence, start_time_str, end_time_str, img_width, img_height):
    ...
    duration = (end_time - start_time).total_seconds()
    if("你身边那些不顾一切选择贫穷爱情的人"in sentence   or "后来都怎么样了" in sentence ):
        input_directory = os.path.join(current_folder, "input")
...
# 将背景音乐叠加在视频的音频上
final_audio = CompositeAudioClip([video_audio.volumex(1.0), bgm.volumex(0.6)])
...
output_folder = os.path.join(current_folder, "output/final_video_with_bgm.mp4")
