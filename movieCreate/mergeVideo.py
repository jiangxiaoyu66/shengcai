...
def find_image_path(folder_path, target_image_name):
    for root, dirs, files in os.walk(folder_path):
        for file in files:
            if file.startswith(target_image_name):
...
def get_emoticon_clip(sentence, duration, imgs_folder, imgIndex):
    try:
        if(imgIndex==0):
            imgPath = find_image_path(imgs_folder, sentence)
            if imgPath is not None:
                emoticon_path = imgPath
            else: 
        ...
        local_emoticon_path = getImgs.download_image(emoticon_path, imgs_folder, sentence)
