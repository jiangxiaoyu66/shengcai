...
def get_emoticon_clip(sentence, duration, imgs_folder, imgIndex):
    try:
        ...
        img = Image.open(local_emoticon_path).convert("RGBA")
        channels = img.getbands()
        mode = img.mode
        print("tongdaoshu", channels, mode)

        gray_image = Image.open(local_emoticon_path).convert('L')
        ...
        # 保存新的RGB图像
        rgb_image.save(f'/Users/sqb/Documents/shengcai/movieCreate/output/binary_image{sentence}.png')

        imgaa = Image.open(f'/Users/sqb/Documents/shengcai/movieCreate/output/binary_image{sentence}.png')


   


        emoticon_clip = ImageClip(f'/Users/sqb/Documents/shengcai/movieCreate/output/binary_image{sentence}.png').set_duration(duration)
